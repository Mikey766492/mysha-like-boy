...
API_TOKEN = "7764083766:AAH5mJ4Xu0-62T0-ZThsB5JvLmfbbqx8leM"
ALLOWED_GROUP_ID = -1002815177809
VIP_USER_ID = 6282709639
...
def join_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📢 Join Channel", url="https://t.me/jexarobot")]
    ])
...
def verify_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Verify For Extra Likes", url="https://shorturl.at/xyz12")]
    ])
...
async def like_handler(msg: Message):
    ...
    if like_usage[region] >= 30 and user_id != VIP_USER_ID:
        await msg.reply(
            f"⚠️ Daily like limit reached for {region} region. Try again tomorrow.",
            reply_markup=join_keyboard()
        )
        return

    wait = await msg.reply("⏳ Sending Likes, Please Wait....")
    url = f"https://anish-likes.vercel.app/like?server_name={region.lower()}&uid={uid}"
    data = await fetch_json(url)

    if not data:
        await wait.edit_text("❌ Failed to send request. Check UID or try later.")
        return
    ...
    await wait.edit_text(
        ...
        f"🎯 Sent By MYSHABOT: {data.get('LikesGivenByAPI', 'N/A')}",
        reply_markup=join_keyboard()
    )
